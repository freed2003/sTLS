...
from derive_secrets import (
    get_client_key_and_iv,
    get_application_key_and_iv,
    client_finished,
)
...
class TLS13Session:
    ...
    def handshake(self):
        ...
        (
            ...
            server_handshake_iv,
        ) = get_client_key_and_iv(shared_key, hello_pre)
        ...
        ) = get_application_key_and_iv(shared_key, total)
        self.wrapper = TLS13Wrapper(
            client_application_key,
            client_application_iv,
            server_application_key,
            server_application_iv,
        ...
        # print(cf.hex())
        enc, tag = parser.AES_encrypt(
            cf, client_handshake_key, client_handshake_iv, header
        )
    ...
    def parse_session_ticket(self, data):
        ...
        ticket["ticket_length"] = int.from_bytes(
            data[13 + ticket["nonce_length"] : 13 + ticket["nonce_length"] + 2], "big"
        )
        ticket["ticket"] = data[
            13
            + ticket["nonce_length"]
        ...
        ticket["ext_length"] = int.from_bytes(
            data[
                13
                + ticket["nonce_length"]
                + 2
                + ticket["ticket_length"] : 13
                + ticket["nonce_length"]
                + 2
                + ticket["ticket_length"]
                + 2
